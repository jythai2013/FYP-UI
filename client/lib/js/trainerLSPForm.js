...
Template.trainerLSPForm.onRendered(function(){
	...
	console.log(tLSP);
	if(tLSP != null && tLSP.length > 0) {
		console.log(lspid);
		Session.set("currentTrainerIDLSPForm2", tLSP);
	}
	...
	if(theLSP != undefined){
		...
		var theTrainer = Meteor.users.findOne({_id:trainerId});
		var TrainerCode = theTrainer.TrainerCode;
		var feedbackAnswerObj = FeedbackAnswers.findOne({assessedOn:TrainerCode});
		console.log(feedbackAnswerObj);
		var settt = "";
		if(feedbackAnswerObj != undefined) settt = feedbackAnswerObj.options;
		Session.set("trainerLSPQns", settt);
});
...
Template.trainerLSPForm.helpers({
	"trainerLSPQns":function(){
		return Session.get("trainerLSPQns");
	},
...
Template.trainerLSPForm.events({
	"click #saveExportTrainerLSP":function(){
		console.log(this);
		var myData = {};
		myData.TrainerName							=	 this.fullName;
		myData.ICFin               			=	 this.userIDType
		myData.SupervisorName           =	 "Marc Sim"
		...
		LSP = Session.get("theLSPSurveyForTheTrainer");
		var sum = 0;
		var num = 0;
		for(i=0;i<this.options.length;i++){
			num += LSP.options[i];
...
function genP2(myData){
  ...
  //   {title: "Country", dataKey: "country"}
	...
	var rows = [
			{"c1": "Trainer Name:", 		"c2": myData.TrainerName, 		"c3": "IC/FIN:", 											"c4":myData.ICFin},
			{"c1": "Supervisor Name:", 	"c2": myData.SupervisorName, 	"c3": "Corrective Action Required:",	"c4":myData.CorrectiveActionRequired},
	...
	doc.addImage(imgData, 'JPEG', 35, 10, 120, 90);
	doc.textAlign("Trainer LSP Form", {align: "center"}, 20, 40);
