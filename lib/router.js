Router.map(function() {
    ...
    this.route('testyAttendance', {
		...
		this.route('pdf', {
			...
			path: '/testy/:file',
			action: function() {
				//var fn = this.params.file.replace(/\.pdf$/, '-' + this.params.path + '.xlsx');
				var fn = this.params.file;
				var data;
				try {
					console.log(fn);
					data2 = fs.readFileSync('/' + fn);
					console.log(data2);
					data3 = fs.readFileSync('' + fn);
					console.log(data3);
					data4 = fs.readFileSync('/build/programs/' + fn);
					console.log(data4);
					data5 = fs.readFileSync('/build/' + fn);
					console.log(data5);
					data1 = fs.readFileSync('/opt/downloads/' + fn);
					console.log(data1);
				} catch(err) {
